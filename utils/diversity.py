...
def niche_entropy(repr_, n_niches = 10):
    ...
    for i in range(1, n_niches+1):
        ...
        distribution = sum(list(map(lambda x : True if (i-1)*stride + min_ <= x < i *stride + min_ else False, num_nodes))) / pop_size
        if distribution > 0:
...
def gsgp_pop_div_from_vectors(sem_vectors): #https://ieeexplore.ieee.org/document/9283096
