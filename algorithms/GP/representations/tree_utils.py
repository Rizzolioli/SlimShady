...
def create_grow_random_tree(
    ...
    if p_c > 0:
        p_terminal = (len(list(TERMINALS.keys())) + len(list(CONSTANTS.keys()))) / (
            len(list(TERMINALS.keys()))
            + len(list(CONSTANTS.keys()))
            + len(list(FUNCTIONS.keys()))
        )
    else:
    ...
    if (depth <= 1 or random.random() < p_terminal) and not first_call:
        ...
        if FUNCTIONS[node]["arity"] == 2:
            ...
            left_subtree = create_grow_random_tree(
                depth - 1, FUNCTIONS, TERMINALS, CONSTANTS, p_c=p_c, first_call=False)
...
def random_subtree(FUNCTIONS):
    ...
    def random_subtree_picker(tree, first_call=True, num_of_nodes=None):
        if isinstance(tree, tuple):
            # Randomly choose to explore left or right or return the current
            # subtree
            if first_call:
            ...
            if FUNCTIONS[tree[0]]["arity"] == 2:
                if first_call:
                    ...
                    # normalizing the probability of choosing left or right
                    # based on the number of nodes in each side
                    subtree_exploration = (
                        1
                        if random.random()
                    ...
                    subtree_exploration = (
                        0 if p < 1 /
                        current_number_of_nodes else (
                            1 if p < len(
                                list(
                                    flatten(
                                        tree[1]))) /
                            current_number_of_nodes else 2))
                ...
                if first_call:
                    ...
                    subtree_exploration = (
                        0 if random.random() < 1 /
                        current_number_of_nodes else 1)

            if subtree_exploration == 0:
                return tree
            elif subtree_exploration == 1:
                return (
                    random_subtree_picker(tree[1], first_call=False)
                    if isinstance(tree[1], tuple)
                    else tree[1]
                )
            elif subtree_exploration == 2:
                return (
                    random_subtree_picker(tree[2], first_call=False)
    ...
    def substitute(tree, target_subtree, new_subtree):
        if tree == target_subtree:
            ...
            if FUNCTIONS[tree[0]]["arity"] == 2:
                ...
                return tree[0], substitute(
                    tree[1], target_subtree, new_subtree)
        else:
            return tree
...
def tree_pruning(TERMINALS, CONSTANTS, FUNCTIONS, p_c=0.3):
    ...
    def pruning(tree, target_depth):
        ...
        if target_depth <= 1 and tree not in list(TERMINALS.keys()):
    ...
    def depth(tree):
        if not isinstance(tree, tuple):
            ...
            if FUNCTIONS[tree[0]]["arity"] == 2:
                ...
                left_depth = depth(tree[1])
                right_depth = 0
            # The depth of the tree is one more than the maximum depth of its
            # subtrees
