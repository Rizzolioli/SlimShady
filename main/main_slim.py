            ...
            for ttress in [False]:
                ...
                for op in ["mul", "sum"]:
                    ...
                    if ttress and sig:
                        ...
                        # getting the log file name according to the used
                        # parameters:
                        algo = f'{algo_name}_{1 +
                                              slim_GSGP_parameters["two_trees"] *
                                              1}_{slim_GSGP_parameters["operator"]}' f'_{sig}'
                        print(sig)
                        print(ttress)
                        print(op)
                        # running each dataset + algo configuration n_runs
                        # times
                        for seed in range(n_runs):
                            ...
                            if isinstance(loader, str):
                                ...
                                # getting the terminals and defining the
                                # terminal-dependant parameters
                            ...
                            slim_GSGP_parameters['p_deflate'] = 1 - \
                                slim_GSGP_parameters['p_inflate']
                            ...
                            # adding the dataset name and algorithm name to the
                            # run info for the logger
                            slim_gsgp_solve_parameters['run_info'] = [
                                algo, unique_run_id, dataset]

                            optimizer = SLIM_GSGP(
                                pi_init=slim_gsgp_pi_init, **slim_GSGP_parameters, seed=seed)
...
log_settings(
    path=os.path.join(
        ...
        "settings.csv"),
    settings_dict=[
        globals()[d] for d in all_params["SLIM_GSGP"]],
