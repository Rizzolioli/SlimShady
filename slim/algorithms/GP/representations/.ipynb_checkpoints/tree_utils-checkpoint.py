...
def _execute_tree(repr_, X, FUNCTIONS, TERMINALS, CONSTANTS):
    ...
    if isinstance(repr_, tuple):  # If it's a function node
        ...
        if FUNCTIONS[function_name]["arity"] == 2:
            ...
            left_result = _execute_tree(left_subtree, X, FUNCTIONS, TERMINALS, CONSTANTS)    #Tree(left_subtree).apply_tree(inputs)
            right_result = _execute_tree(right_subtree, X, FUNCTIONS, TERMINALS, CONSTANTS)    #Tree(right_subtree).apply_tree(inputs)
