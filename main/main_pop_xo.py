...
# (pop_size, n_iter) — all same total budget ~200k evaluations
pop_configs = [
    (200,  1000),   # same budget, doubled pop
    (500,   400),   # front-loaded, aggressive
    (1000,  200),   # maximum front-loading
]
p_xo_values    = [0.7]   # baseline vs best early-convergence setting
max_head_depth = 17
...
_LOG_PATH      = os.path.join(os.path.dirname(__file__), "log", "results_pop_xo_15052026.csv")
_SETTINGS_PATH = os.path.join(os.path.dirname(__file__), "log", "settings_pop_xo.csv")

########################################################################################################################

# COMPLETED-RUN DETECTION
# Different tasks have different n_iter, so we store the max generation seen per
# (algo, loader, seed) and skip if it already reached that task's n_iter.
...
def run_one(task):
    (loader, sig, ttrees, op, pop_size, n_iter, p_xo, seed,
...
if __name__ == '__main__':
    ...
    for loader in data_loaders:
        ...
        for (sig, ttrees, op, gsgp) in variants:
            ...
            for (pop_size, n_iter) in pop_configs:
                for p_xo in p_xo_values:
                    algo = f'{algo_base}_pop{pop_size}_iter{n_iter}_pxo{p_xo}'
    ...
    with ProcessPoolExecutor(max_workers=N_JOBS) as executor:
        ...
        for i, fut in enumerate(as_completed(futures), 1):
            ...
            try:
                ...
                print(f"[{i}/{len(tasks)}] FAILED [{t[0]}] {t[7]} seed={t[7]}: {exc}")
    ...
    log_settings(
        path=_SETTINGS_PATH,
        settings_dict=[
            {"n_runs": n_runs, "log": 8, "elitism": True},
            {"pop_configs": str(pop_configs), "p_xo_values": str(p_xo_values)},
            {"max_head_depth": max_head_depth, "head_xo_freq": None,
