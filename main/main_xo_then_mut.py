...
main_xo_then_mut.py
===================
Phase-switched experiment:
  gens   1 – 50  : head XO only      (p_xo = 1.0)
  gens  51 – 400 : inflate/deflate only (p_xo = 0.0)

The switch is implemented via p_xo_schedule, reusing the existing
slim_gsgp.py hook introduced for the cosine² decay experiment.

Config: pop=500, n_iter=400, max_head_depth=17
Variants: SLIM+2SIG, SLIM*ABS, SLIM*1SIG
Datasets: all 6 benchmarks, 5 seeds
Log: level 9 (operator counts per gen + _sem_gen.csv companion)
...
n_iter         = 400
max_head_depth = 17
XO_PHASE_END   = 50      # last gen with p_xo = 1.0
...
_LOG_PATH = os.path.join(os.path.dirname(__file__), "log",
                         f"results_xo_then_mut_{_date}.csv")
...
def run_one(task):
    ...
    def _phase_schedule(it, n_iter):
        return 1.0 if it <= xo_phase_end else 0.0
    ...
    optimizer = SLIM_GSGP(
        ...
        p_xo=1.0,          # initial value; schedule overrides every gen
        p_m=0.0,
        pop_size=pop_size,
...
if __name__ == '__main__':
    print(f"Phase schedule: XO-only gens 1-{XO_PHASE_END}, "
          f"inflate/deflate-only gens {XO_PHASE_END+1}-{n_iter}")
    print(f"Config: pop={pop_size}, n_iter={n_iter}, hd={max_head_depth}, "
          f"seeds={n_runs}, datasets={len(data_loaders)}")
    print()
    ...
    for loader in data_loaders:
        ...
        for (sig, ttrees, op, gsgp) in variants:
            ...
            algo = f'{algo_base}_pop{pop_size}_iter{n_iter}_xo{XO_PHASE_END}_mut{n_iter - XO_PHASE_END}'
