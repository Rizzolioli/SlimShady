...
Usage:
    ...
    python utils/benchmark_operators.py

    Or from the project root:
    cd main && python ../utils/benchmark_operators.py
"""
...
N_FEATURES = 10
TERMINALS = [f'x_{i}' for i in range(N_FEATURES)]
...
def create_multi_block_individual(X, X_test=None, n_blocks=5, init_depth=6, p_c=0.1, reconstruct=True):
    ...
    trees = rhh(init_pop_size=n_blocks, init_depth=init_depth,
                FUNCTIONS=FUNCTIONS, TERMINALS=TERMINALS,
                CONSTANTS=CONSTANTS, p_c=p_c)
...
def benchmark_inflate_mutation(individual, X, X_test, n_runs=100):
    """
    ...
    for _ in range(n_runs):
        ...
        ind_copy = create_multi_block_individual(X, X_test, n_blocks=individual.size)

        # Measure time
        start_time = time.perf_counter()
        ms = np.random.uniform(0, 1)
        _ = inflate_mutator(ind_copy, ms, X, max_depth=8, p_c=0.1,
...
def benchmark_deflate_mutation(individual, X, X_test, n_runs=100):
    """
    ...
    ----------
    individual : Individual
        The individual to mutate (must have size > 1)
    X : torch.Tensor
        Training data
    X_test : torch.Tensor
        Testing data
    ...
    for _ in range(n_runs):
        # Create a copy of the individual with multiple blocks
        ind_copy = create_multi_block_individual(X, X_test, n_blocks=max(individual.size, 3))

        # Measure time
        start_time = time.perf_counter()
        _ = deflate_mutation(ind_copy, reconstruct=True)
        end_time = time.perf_counter()
...
def benchmark_donor_crossover(individual1, individual2, X, X_test, n_runs=100):
    """
    ...
    for _ in range(n_runs):
        ...
        ind1_copy = create_multi_block_individual(X, X_test, n_blocks=individual1.size)
        ind2_copy = create_multi_block_individual(X, X_test, n_blocks=individual2.size)

        # Measure time
        start_time = time.perf_counter()
        _ = donor_xo(ind1_copy, ind2_copy, reconstruct=True)
        end_time = time.perf_counter()
...
def benchmark_best_donor_crossover(individual1, individual2, X, X_test, n_runs=100):
    """
    ...
    for _ in range(n_runs):
        ...
        # Measure time
        start_time = time.perf_counter()
        _ = best_d_xo(ind1_copy, ind2_copy, reconstruct=True)
        end_time = time.perf_counter()
...
def run_benchmark(n_samples=100, n_features=10, n_runs=100, n_blocks=5):
    ...
    # Create random individuals for testing
    print("Creating random individuals...")
    individual_single = create_random_individual(X_train, X_test, reconstruct=True)
    individual_multi1 = create_multi_block_individual(X_train, X_test, n_blocks=n_blocks, reconstruct=True)
    individual_multi2 = create_multi_block_individual(X_train, X_test, n_blocks=n_blocks, reconstruct=True)

    print(f"  - Single-block individual: size={individual_single.size}")
    print(f"  - Multi-block individual 1: size={individual_multi1.size}")
    print(f"  - Multi-block individual 2: size={individual_multi2.size}")
    ...
    results['Inflate Mutation'] = benchmark_inflate_mutation(
        individual_multi1, X_train, X_test, n_runs=n_runs
    )
    ...
    results['Donor Crossover'] = benchmark_donor_crossover(
        individual_multi1, individual_multi2, X_train, X_test, n_runs=n_runs
    )
