"""
main_slim_r2.py — same as main_slim_normalized.py but trains with R² as the
fitness function (maximised) instead of RMSE (minimised).

All 14 SLIM variants are included. Results go to:
  main/log/results_r2_generations.csv
  main/log/results_r2_simplification.csv

Resume detection is active: completed (algo, dataset, seed) triples are
skipped on restart.
"""
...
_GEN_LOG_NAME  = "results_r2_generations.csv"
_SIMP_LOG_NAME = "results_r2_simplification.csv"

_GEN_HEADER = [
    'algo', 'run_id', 'dataset', 'seed', 'generation',
    'train_fitness', 'timing', 'nodes',
    'test_fitness', 'nodes_count', 'm_phi', 'no', 'nnao', 'nnaoc', 'mae', 'r2', 'rmse', 'log_level',
]
...
def run_experiment_worker(dataset, variant_idx, seed, run_id_str,
    ...
    from evaluators.fitness_functions import neg_r2
...
if __name__ == "__main__":
    ...
    # Resume detection: skip already-completed (algo, dataset, seed) triples
    completed_runs = set()
