...
from algorithms.SLIM_GSGP.operators.mutators import inflate_mutation, deflate_mutation, more_blocks_deflate_mutation
import torch
import numpy as np
import random
import csv
...
def inflate_deflate_study(blocks,
                           X_train,
    ...
    for _ in range(blocks-1):
        ...
        if any(inflated_individual.size == def_ for def_ in deflate_on):
            for j in range(1, inflated_individual.size-1):
                ...
                if log > 0:
                    log_row = [dataset_name, algorithm_name, seed, 'deflate', inflated_individual.size,
                               deflated_individual.nodes_count, deflated_individual.depth, j,
                               train_var, test_var, deflated_individual.full_train_semantics[0].float()]
...
def more_blocks_deflate_study(blocks,
                           X_train,
                           FUNCTIONS, TERMINALS, CONSTANTS,
                           deflate_on,
                           X_test=None,
    ...
    individual = Individual(
        collection=[GSGPTree(create_grow_random_tree(initial_depth, FUNCTIONS, TERMINALS, CONSTANTS, p_c=0),
                             reconstruct=True,
                             train_semantics=None,
                             test_semantics=None
                             )],
    ...
    for _ in range(blocks - 1):
        ...
        if any(inflated_individual.size == def_ for def_ in deflate_on):
            for j in range(1, inflated_individual.size - 1):
                ...
                deflated_individual = more_blocks_deflate_mutation(inflated_individual,
                                                                   reconstruct=False,
                                                                   allow_bt=False,
                                                                   blocks_to_remove=j)
                ...
                if X_test is not None:
                    test_var = torch.sum(
                        deflated_individual.full_test_semantics != inflated_individual.full_test_semantics).item() / \
                               individual.full_test_semantics.shape[0]
                else:
                    test_var = 0
