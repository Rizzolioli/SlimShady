...
def nested_depth_calculator(operator, depths):
    ...
    elif operator.__name__ == 'tt_delta_normalized_sum':
        depths[0] += 3
        depths[1] += 3

    elif operator.__name__ == 'tt_delta_normalized_mul':
        depths[0] += 4
        depths[1] += 4

    elif operator.__name__ == 'ot_delta_normalized_sum':
        depths[0] += 5

    elif operator.__name__ == 'ot_delta_normalized_mul':
        depths[0] += 6

    elif operator.__name__ == 'tt_delta_normrob_sum':
        depths[0] += 3
        depths[1] += 3

    elif operator.__name__ == 'tt_delta_normrob_mul':
        depths[0] += 4
        depths[1] += 4

    elif operator.__name__ == 'tt_delta_norm12_sum':
        depths[0] += 6
        depths[1] += 6

    elif operator.__name__ == 'tt_delta_norm12_mul':
        depths[0] += 7
        depths[1] += 7
...
def nested_nodes_calculator(operator, nodes):
    ...
    extra_operators_nodes = \
        [5, nodes[-1]] if name == 'geometric_crossover' else \
        [7]  if name == 'ot_delta_sum_True'        else \
        [11] if name == 'ot_delta_mul_False'        else \
        [9]  if name in ('ot_delta_sum_False', 'ot_delta_mul_True') else \
        [6]  if name == 'tt_delta_mul'              else \
        [4]  if name == 'tt_delta_sum'              else \
        [5]  if name == 'tt_delta_normalized_sum'   else \
        [7]  if name == 'tt_delta_normalized_mul'   else \
        [10] if name == 'ot_delta_normalized_sum'   else \
        [12] if name == 'ot_delta_normalized_mul'   else \
        [5]  if name == 'tt_delta_normrob_sum'      else \
