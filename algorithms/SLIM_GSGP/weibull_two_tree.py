...
class WeibullTwoTree:

    def __init__(self, pi_init, initializer, selector, ms, find_elit_func,
                 p_m=1.0, pop_size=100, seed=0, settings_dict=None):
        ...
        # bind static registries used by GSGP Tree
    ...
    def _make_two_tree_individual(self, t1, t2):
        return Individual(collection=[Tree(t1, None, None, True),
                                      Tree(t2, None, None, True)],
    ...
    def _mutate_tree(self, base_tree, X_train, X_test=None, reconstruct=True):
        ...
        if X_test is not None:
            # compute test semantics if requested in main loop
    ...
    def solve(self, X_train, X_test, y_train, y_test, curr_dataset, run_info,
              n_iter=20, elitism=True, log=0, verbose=0, test_elite=False, log_path=None,
              reconstruct=True, n_elites=1):
        ...
        # initialization: build pairs of trees per individual
        ...
        population.evaluate(weibull_cindex_loss, y=y_train, operator='sum')  # operator ignored for this ffunction
        ...
        if test_elite:
            ...
            self.elite.evaluate(weibull_cindex_loss, y=y_test, testing=True, operator='sum')
        ...
        # evolution: per-tree geometric mutation keeping exactly two trees per individual
        for it in range(1, n_iter + 1, 1):
            ...
            while len(offs_pop) < self.pop_size:
                ...
                if random.random() < self.p_m:
                    # mutate both component trees independently, keep pair size = 2
                    ...
                    off = Individual(collection=[off_t1, off_t2] if reconstruct else None,
                                     ...
                                     reconstruct=reconstruct)
                    # sizes and depths recomputed by Individual on reconstruct path; copy minimal metadata when not reconstruct
                    if not reconstruct:
                        off.size = 2
                else:
                    # copy parent
            ...
            offs_pop.evaluate(weibull_cindex_loss, y=y_train, operator='sum')
            population = offs_pop
